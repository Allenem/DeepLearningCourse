...
if __name__ == '__main__' and sys.argv[1] == 'train':
    ...
    for i in range(0, NUM_CLASSES):
        # i is the label
        dir = './dataset-train&val/training-set/province/%s/' % i
    ...
    val_count = 0
    for i in range(0, NUM_CLASSES):
        dir = './dataset-train&val/validation-set/province/%s/' % i
        for rt, dirs, files in os.walk(dir):
            for filename in files:
                val_count += 1
    ...
    with tf.Session() as sess:
        ...
        W_fc1 = tf.Variable(tf.truncated_normal(
            [32 / 2 * 40 / 2 * 32, 512], stddev=0.1), name="W_fc1")
        b_fc1 = tf.Variable(tf.constant(0.1, shape=[512]), name="b_fc1")
        h_pool2_flat = tf.reshape(L2_pool, [-1, 32 / 2 * 40 / 2 * 32])
