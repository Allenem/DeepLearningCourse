...
if __name__ == '__main__' and sys.argv[1] == 'train':
    ...
    # 1.Train-set: Get image number through the first traversal
    input_count = 0
    for i in range(0, NUM_CLASSES):
        dir = './dataset-train&val/training-set/letter/%s/' % i
        for rt, dirs, files in os.walk(dir):
            for filename in files:
                input_count += 1

    # 2.Train-set: Define binary img with dimension 1*size, one-hot label with dim 1*classes, through the second traversal
    ...
    val_count = 0
    for i in range(0, NUM_CLASSES):
        dir = './dataset-train&val/validation-set/letter/%s/' % i
        for rt, dirs, files in os.walk(dir):
            for filename in files:
                val_count += 1
...
if __name__ == '__main__' and sys.argv[1] == 'predict':
    ...
    with tf.Session() as sess:
        ...
        # predict 7.bmp ~ 11.bmp or 12.bmp ~ 17.bmp
        ...
        for n in range(7, 12):
            ...
            for j in range(NUM_CLASSES):
                ...
                if (result[0][j] > max3) and (result[0][j] <= max2):
                    ...
                    max3_index = j
                    continue
            license_letter = license_letter + LETTERS[max1_index]
            print("概率： [%s %0.2f%%]  [%s %0.2f%%]  [%s %0.2f%%]" % (LETTERS[max1_index],
                                                                    max1*100, LETTERS[max2_index], max2*100, LETTERS[max3_index], max3*100))
        print("车牌编号是: [%s]" % license_letter)
